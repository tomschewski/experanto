...
def test_nearest_neighbor_screen_interpolation(fps):
    with create_screen_data(n_frames=20, frame_shape=(32, 32), t_end=4.0, fps=fps) as timestamps:
        screen_interp = Interpolator.create("tests/screen_data")
        assert isinstance(screen_interp, ScreenInterpolator), "Expected ScreenInterpolator"
        ...
        assert np.allclose(interp, expected_frames, atol=1e-5), "Nearest neighbor interpolation mismatch"
